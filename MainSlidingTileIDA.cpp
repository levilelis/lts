...
int main(int argc, char **argv)
{
	if( argc != 4 )
	{
		...
		cout << "USAGE: SLIDA size width input-problems" << endl;
		exit(-1);
	}
...
#endif
	{
		...
		sIda = clock();
		idaStar->idaStar(state);
		endIda = clock();

		long nodesExpanded = idaStar->getTotalNodesExpanded();
		float cost = idaStar->getCost();
